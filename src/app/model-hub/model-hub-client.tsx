...
const ModelCard = memo(function ModelCard({ item }: { item: ModelHubItem }) {
...
export function ModelHubClient({ data }: { data: ModelHubResponse }) {
    ...
    <main className="mx-auto max-w-7xl px-6 pb-20 pt-10">
      <header className="mb-8">
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
          聚合并人工策展 GitHub 上与医学影像、临床大模型、生信组学、AI
          药物发现相关的开源项目，按相关性、活跃度、可复用信号和人工理解排序。
        </p>
      </header>
      ...
      <section className="mb-6 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <div className="mt-1 text-2xl font-bold text-slate-950">
            {activeCategory ? labelFor(CATEGORY_LABELS, activeCategory) : "全部"}
          </div>
        </div>
      </section>

      <nav className="mb-6 flex flex-wrap gap-2" aria-label="模型 Hub 分类">
        <CategoryButton
          label="全部"
        ...
        {categories.map((category) => (
          <CategoryButton
            key={category}
            label={labelFor(CATEGORY_LABELS, category)}
      ...
      {filteredItems.length ? (
        <section
          className={
            isPending
              ? "grid gap-5 opacity-80 transition-opacity lg:grid-cols-2"
              : "grid gap-5 transition-opacity lg:grid-cols-2"
          }
        >
